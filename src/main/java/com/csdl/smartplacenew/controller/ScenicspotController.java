...
import com.csdl.smartplacenew.vo.ScenicspotVo;
...
public class ScenicspotController {
    ...
    @Resource
    ScenicspotNaUrMapper scenicspotNaUrMapper;
    ...
    private ResultVO add( HttpServletRequest request) throws IOException {
        ...
        Integer scenicspotMapperMaxIdByAll=scenicspotMapper.getMaxIdByAll(currentUserId);

        if(scenicspotMapperMaxIdByAll==null){

            scenicspotMapperMaxIdByAll=1;

        }

        if(scenicspotMapperMaxIdByAll>0){

            scenicspotMapperMaxIdByAll=scenicspotMapperMaxIdByAll+1;

        }

        scenicspotMapper.addScenicspot(name,lonandlat,type,currentUserId+"/"+scenicspotMapperMaxIdByAll,imgVirtualPath,1,
        ...
        Scenicspot scenicspot=scenicspotMapper.getScenicspotByMaxId(currentUserId);


        List<Scenicspot> scenicspotList=scenicspotMapper.getScenicspotByName(name,currentUserId);

        if(scenicspotList.size()==1){

            scenicspotMapper.addCountOfScenicspot(name,scenicspotList.size(),0,"scenicspot",1,0,currentUserId);
        }

        if(scenicspotList.size()>1){

            countMapper.updatePlateCount2(name,"scenicspot",currentUserId);

        }



        return ResultVOUtil.success(scenicspot.getId());
    ...
    @ApiOperation(value = "删除景区", notes = "")
    @ApiImplicitParams({

            @ApiImplicitParam(name = "id", value = "景区id", required = true, dataType = "integer", paramType = "query"),
    ...
    private ResultVO del(HttpServletRequest request) {
        ...
        Scenicspot scenicspot=scenicspotMapper.getScenicspotById(id,currentUserId);

        List<Scenicspot> scenicspotList=scenicspotMapper.getScenicspotByName(scenicspot.getName(),currentUserId);

        if(scenicspotList.size()==1){
            countMapper.updatePlateCount4(scenicspot.getName(),"scenicspot",currentUserId);
        }

        if(scenicspotList.size()>1){

            countMapper.updatePlateCount3(scenicspot.getName(),"scenicspot",currentUserId);

        }


        scenicspotMapper.updateScenicspotStatus(id,currentUserId);

        return  ResultVOUtil.success("成功删除");
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "englishword", value = "英语内容", required = true, dataType = "integer", paramType = "query"),
    ...
    private ResultVO update( HttpServletRequest request) throws IOException {
        ...
        if(request.getParameter("textofmater")!=null&&!request.getParameter("textofmater").equals("")){
            ...
            textofmater=scenicspot.getTextofmater();
        }
        ...
        if(request.getParameter("yearofesent")!=null&&!request.getParameter("yearofesent").equals("")){
            ...
            yearofesent=scenicspot.getYearofesent();
        }
        ...
        if(request.getParameter("mainteunit")!=null&&!request.getParameter("mainteunit").equals("")){
            ...
            mainteunit=scenicspot.getMainteunit();
        }
        ...
        if(request.getParameter("numofmaint")!=null&&!request.getParameter("numofmaint").equals("")){
            ...
            numofmaint=scenicspot.getNumofmaint();
        }
        ...
        if(request.getParameter("maintperiod")!=null&&!request.getParameter("maintperiod").equals("")){
            ...
            maintperiod=scenicspot.getMaintperiod();
        }
        ...
        if(request.getParameter("maintcost")!=null&&!request.getParameter("maintcost").equals("")){
            ...
            maintcost=scenicspot.getMaintcost();
        }
        ...
        if(request.getParameter("maintaccounts")!=null&&!request.getParameter("maintaccounts").equals("")){
            ...
            maintaccounts=scenicspot.getMaintaccounts();
        }
        ...
        if(request.getParameter("englishword")!=null&&!request.getParameter("englishword").equals("")){
            ...
            englishword=scenicspot.getEnglishword();
        }
        ...
        String imgVirtualPath=null;
            ...
            imgVirtualPath=scenicspot.getVoiceurl();


        scenicspotMapper.updateScenicspotById(name,lonandlat,type,scenicspot.getUrl(),imgVirtualPath,
        ...
        countMapper.updateCountName(name,scenicspot.getName(),"scenicspot",currentUserId);

        feedbackMapper.updateFeddBackFeedbackpath(name,scenicspot.getName(),"景区管理",currentUserId);
    ...
    @ApiOperation(value = "获取景区信息", notes = "")
    @ApiImplicitParams({
    ...
    private ResultVO getList1(HttpServletRequest request) {
        ...
        if(key!=null&&!key.equals("")){
            ...
            Integer count=scenicspotMapper.getScenicspotByKeyCount(currentUserId,"%"+key+"%");

            PageHelper.startPage(page,size);

            List<Scenicspot>  scenicspotList=scenicspotMapper.getScenicspotByKey(currentUserId,"%"+key+"%");
            ...
            for(int i=0;i<scenicspotList.size();i++){
                ...
                scenicspotVo.setName(scenicspot.getName());

                 if(scenicspot.getLonandlat()!=null&&!scenicspot.getLonandlat().equals("")){

                     String[] strings=scenicspot.getLonandlat().split(",");
                     scenicspotVo.setLon(strings[0]);
                     scenicspotVo.setLat(strings[1]);

                 }

                scenicspotVo.setType(scenicspot.getType());
                scenicspotVo.setUrl(scenicspot.getUrl());

                scenicspotVo.setMandarinWord(scenicspot.getMandarinword());
                scenicspotVo.setEnglishWord(scenicspot.getEnglishword());

                List<String> stringList=audioMapper.getAudioByScenicspotid(scenicspot.getId(),currentUserId);
                ...
                if(stringList!=null&&!stringList.equals("")&&!stringList.equals("null")){
                    ...
                    for(int k=0;k<stringList.size();k++){
                        ...
                        if(stringList.get(k).lastIndexOf("mandarin")!=-1) {
                            scenicspotVo.setMandarinVoice(stringList.get(k));
                        }

                        if(stringList.get(k).lastIndexOf("english")!=-1) {
                            scenicspotVo.setEnglishVoice(stringList.get(k));
                        }

                        if(stringList.get(k).lastIndexOf("dialect")!=-1) {
                            scenicspotVo.setDialectVoice(stringList.get(k));
                        }
                ...
                List<String>  picture=pictureMapper.getPictureByScenicspotid(scenicspot.getId(),currentUserId);

                if(picture!=null){
                    scenicspotVo.setPictureUrl(picture);
                }


                scenicspotVo.setTextOfMater(scenicspot.getTextofmater());
                scenicspotVo.setSize(scenicspot.getSize());
                scenicspotVo.setYearOfEsent(scenicspot.getYearofesent());
                scenicspotVo.setMainteUnit(scenicspot.getMainteunit());
                scenicspotVo.setNumofMaint(scenicspot.getNumofmaint());
                scenicspotVo.setMaintPeriod(scenicspot.getMaintperiod());
                scenicspotVo.setMaintCost(scenicspot.getMaintcost());
                scenicspotVo.setMaintAccounts(scenicspot.getMaintaccounts());

                scenicspotVo.setSharelinks(scenicspot.getSharelinks());
            ...
            listVO.setList(scenicspotVoList);
            ...
            Integer count=scenicspotMapper.getScenicspotByKeyCount(currentUserId,null);

            PageHelper.startPage(page,size);

            List<Scenicspot>  scenicspotList=scenicspotMapper.getScenicspotByKey(currentUserId,null);
    ...
    @ApiOperation(value = "通过id获取景区信息", notes = "")
    @ApiImplicitParams({
    ...
    private ResultVO getListById(HttpServletRequest request) {
        ...
        scenicspotVo.setId(scenicspot.getId());
        scenicspotVo.setName(scenicspot.getName());
        scenicspotVo.setLonandlat(scenicspot.getLonandlat());
        ...
        return ResultVOUtil.success(scenicspotVo);
    ...
    @ApiOperation(value = "添加景区维护信息", notes = "")
    @ApiImplicitParams({
    ...
    @PostMapping("/addMaintrecordsOfScen")
    private ResultVO addMaintrecordsOfScen(HttpServletRequest request) throws  IOException {
        ...
        scenicspotMapper.addMaintrecordsOfScen("景区",null,maintcontent,maintcompany,contacts,contactsnum,
    ...
    @ApiOperation(value = "获取所有经纬度信息", notes = "")
    @ApiImplicitParams({
    ...
    private ResultVO getLonAndLat(HttpServletRequest request) {
        ...
        List<String> stringList=scenicspotMapper.getLonAndLat(currentUserId);
    ...
    @ApiOperation(value = "通过经纬度获取信息", notes = "")
    @ApiImplicitParams({
    ...
    @GetMapping("/getList2")
    private ScenicspotVo getList2(HttpServletRequest request) {
        ...
        Scenicspot scenicspot=scenicspotMapper.getScenicpoByLonAndLat(key,currentUserId);
    ...
    @GetMapping("/getScenicspotByUrl")
    private ScenicspotVo getScenicspotByUrl(HttpServletRequest request) {
    ...
    @ApiOperation(value = "获取景区统计数量" , notes = "")
    @ApiImplicitParams({
    ...
    @GetMapping("/getScenicspotCountByName")
    private ResultVO getScenicspotCountByName(HttpServletRequest request) {
        ...
        if(name!=null){
            ...
            Integer count=countMapper.getCountOfAll("%"+name+"%","scenicspot",currentUserId);
            PageHelper.startPage(page,size);
            List<Count>  plateNumberCount=countMapper.getPlateNumberCount("%"+name+"%","scenicspot",currentUserId);
            ...
            Integer count=countMapper.getCountOfAll(null,"scenicspot",currentUserId);
            PageHelper.startPage(page,size);
            List<Count>  plateNumberCount=countMapper.getPlateNumberCount(null,"scenicspot",currentUserId);
    ...
    private ResultVO getNameAndUrl(HttpServletRequest request) {
        ...
        List<ScenicspotNaUr> scenicspotList=scenicspotNaUrMapper.getNameAndUrl(currentUserId);

        return ResultVOUtil.success(scenicspotList);
