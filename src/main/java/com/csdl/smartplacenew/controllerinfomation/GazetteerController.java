...
import com.csdl.smartplacenew.informationvo.GazetteerVo;
import com.csdl.smartplacenew.mapper.GazetteerMapper;
...
public class GazetteerController {
    ...
    @Resource
    GazetteerMapper gazetteerMapper;

    @Resource
    UserMapper userMapper;


    @ApiOperation(value = "添加地名志", notes = "")
    @ApiImplicitParams({
    ...
    private ResultVO add(@ApiParam("file") MultipartFile file, HttpServletRequest request) throws IOException {
        ...
        if(file!=null) {
            ...
            String fileRealPath = ConfigBean.sourceRealPath + "information" + File.separator + "Gazetteer" + File.separator;
            ...
            String imgVirtualPath = ConfigBean.sourceVirtualPath + "information/" + "Gazetteer/" + filename;
            gazetteerMapper.addGazetteer(name, imgVirtualPath, 1,currentUserId);
        }
        else{
            gazetteerMapper.addGazetteer(name, null, 1,currentUserId);
        }


        Gazetteer gazetteer=gazetteerMapper.getGazetteerByMaxId(currentUserId);


        return ResultVOUtil.success(gazetteer);
    ...
    @ApiOperation(value = "添加地名志内容", notes = "")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "地名志id", required = true, dataType = "integer", paramType = "query"),
    ...
    private void addTxt( HttpServletRequest request) throws IOException {
        ...
        String filepath =  ConfigBean.sourceRealPath+"txt"+ File.separator+"Gazetteer"+File.separator;
        ...
        String filepath2=  ConfigBean.sourceRealPath+"txt"+ File.separator+"Gazetteer"+File.separator+"Gazetteer"+id+".txt";
    ...
    @ApiOperation(value = "上传地名志内容文件", notes = "")
    @ApiImplicitParams({
    ...
    private String addTxtFile(@ApiParam("file") MultipartFile file, HttpServletRequest request) throws IOException {
        ...
        String filepath2=  ConfigBean.sourceRealPath+"informapict"+ File.separator+"Gazetteer"+File.separator;
        ...
        String realPath= ConfigBean.sourceVirtualPath+"informapict/"+"Gazetteer/"+filename;
    ...
    @ApiOperation(value = "修改地名志内容", notes = "")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "地名志id", required = true, dataType = "integer", paramType = "query"),
            @ApiImplicitParam(name = "txt", value = "内容", required = true, dataType = "string", paramType = "query"),
    })
    ...
    private void updateTxt( HttpServletRequest request) throws IOException {
        ...
        String filepath =  ConfigBean.sourceRealPath+"txt"+ File.separator+"Gazetteer"+File.separator+"Gazetteer"+id+".txt";
    ...
    @ApiOperation(value = "查询地名志", notes = "")
    @ApiImplicitParams({
    ...
    private ResultVO getList1(HttpServletRequest request) throws IOException {
        ...
        if(name!=null&&!name.equals("")){
            ...
            Integer count=gazetteerMapper.getGazetteerCount("%"+name+"%",currentUserId);

            PageHelper.startPage(page,size);

            List<Gazetteer> gazetteerList=gazetteerMapper.selectGazetteer2("%"+name+"%",currentUserId);
...
//                BufferedReader br = new BufferedReader(reader); // 建立一个对象，它把文件内容转成计算机能读懂的语言
            ...
            Integer count = gazetteerMapper.getGazetteerCount(null, currentUserId);

            PageHelper.startPage(page, size);

            List<Gazetteer> gazetteerList = gazetteerMapper.selectGazetteer2(null, currentUserId);
...
//                BufferedReader br = new BufferedReader(reader); // 建立一个对象，它把文件内容转成计算机能读懂的语言
    ...
    @ApiOperation(value = "删除地名志内容", notes = "")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "地名志id", required = true, dataType = "integer", paramType = "query"),
    ...
    private ResultVO del( HttpServletRequest request) throws IOException {
        ...
        gazetteerMapper.delGazetteer(id,currentUserId);
    ...
    private ResultVO update(@ApiParam("file") MultipartFile file, HttpServletRequest request) throws IOException {
        ...
        Gazetteer gazetteer=gazetteerMapper.getGazetteerById2(id,currentUserId);
        ...
        if(file!=null) {
            ...
            if(gazetteer.getPictureurl()!=null){

                if(gazetteer.getPictureurl()!=null&&!gazetteer.getPictureurl().equals("")) {
                    String[] strings = gazetteer.getPictureurl().split("\\/");

                    File file1 = new File(ConfigBean.sourceRealPath + "information" + File.separator + "Gazetteer" + File.separator + strings[5]);
                ...
                gazetteerMapper.updateGazetteer(name,imgVirtualPath,id,currentUserId);

                Gazetteer gazetteer1=gazetteerMapper.getGazetteerById2(id,currentUserId);

                return  ResultVOUtil.success(gazetteer1);
            ...
            gazetteerMapper.updateGazetteer(name,gazetteer.getPictureurl(),id,currentUserId);
