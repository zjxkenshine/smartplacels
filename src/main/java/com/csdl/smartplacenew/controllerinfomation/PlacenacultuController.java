...
import com.csdl.smartplacenew.informationvo.PlacenacultuVo;
import com.csdl.smartplacenew.mapper.PlacenacultuMapper;
...
@Api(description = "地名文化")
@RestController
@RequestMapping("/Placenacultu")
public class PlacenacultuController {


    @Resource
    PlacenacultuMapper placenacultuMapper;

    @Resource
    UserMapper userMapper;


    @ApiOperation(value = "添加地名文化", notes = "")
    @ApiImplicitParams({
    ...
    private ResultVO add(@ApiParam("file") MultipartFile file, HttpServletRequest request) throws IOException {
        ...
        if(file!=null) {
            ...
            String fileRealPath = ConfigBean.sourceRealPath + "information" + File.separator + "Placenacultu" + File.separator;
            ...
            String imgVirtualPath = ConfigBean.sourceVirtualPath + "information/" + "Placenacultu/" + filename;
            placenacultuMapper.addPlacenacultu(name, type2, imgVirtualPath, 1,currentUserId,abbreviation);
        }
        else{
            placenacultuMapper.addPlacenacultu(name, type2, null, 1,currentUserId,abbreviation);
        }

        Placenacultu placenacultu=placenacultuMapper.getPlacenacultuByMaxId(currentUserId);


        return ResultVOUtil.success(placenacultu);
    ...
    @ApiOperation(value = "添加地名文化内容", notes = "")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "地名文化id", required = true, dataType = "integer", paramType = "query"),
    ...
    private void addTxt( HttpServletRequest request) throws IOException {
        ...
        String filepath =  ConfigBean.sourceRealPath+"txt"+ File.separator+"Placenacultu"+File.separator;
        ...
        String filepath2=  ConfigBean.sourceRealPath+"txt"+ File.separator+"Placenacultu"+File.separator+"Placenacultu"+id+".txt";
    ...
    @ApiOperation(value = "上传地名文化内容文件", notes = "")
    @ApiImplicitParams({
    ...
    private String addTxtFile(@ApiParam("file") MultipartFile file, HttpServletRequest request) throws IOException {
        ...
        String filepath2=  ConfigBean.sourceRealPath+"informapict"+ File.separator+"Placenacultu"+File.separator;
        ...
        String realPath= ConfigBean.sourceVirtualPath+"informapict/"+"Placenacultu/"+filename;
    ...
    @ApiOperation(value = "修改地名文化内容", notes = "")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "地名文化id", required = true, dataType = "integer", paramType = "query"),
            @ApiImplicitParam(name = "txt", value = "内容", required = true, dataType = "string", paramType = "query"),
    })
    ...
    private void updateTxt( HttpServletRequest request) throws IOException {
        ...
        String filepath =  ConfigBean.sourceRealPath+"txt"+ File.separator+"Placenacultu"+File.separator+"Placenacultu"+id+".txt";
    ...
    @ApiOperation(value = "查询地名文化", notes = "")
    @ApiImplicitParams({
    ...
    private ResultVO getList1(HttpServletRequest request) throws IOException {
        ...
        if(name!=null&&!name.equals("")){
            ...
            Integer count=placenacultuMapper.getPlacenacultuCount("%"+name+"%",type,currentUserId);

            PageHelper.startPage(page,size);

            List<Placenacultu> placenacultuList=placenacultuMapper.selectPlacenacultu2("%"+name+"%",type,currentUserId);

            List<PlacenacultuVo> placenacultuVoArrayList=new ArrayList<>();

            for(int i=0;i<placenacultuList.size();i++){

                Placenacultu placenacultu=placenacultuList.get(i);

                PlacenacultuVo placenacultuVo=new PlacenacultuVo();
...
//                BufferedReader br = new BufferedReader(reader); // 建立一个对象，它把文件内容转成计算机能读懂的语言
                ...
                placenacultuVoArrayList.add(placenacultuVo);

            }

            ListVO listVO=new ListVO();
            listVO.setCount(count);
            listVO.setList(placenacultuVoArrayList);
            ...
            Integer count=placenacultuMapper.getPlacenacultuCount(null,type,currentUserId);

            PageHelper.startPage(page,size);

            List<Placenacultu> placenacultuList=placenacultuMapper.selectPlacenacultu2(null,type,currentUserId);
...
//                BufferedReader br = new BufferedReader(reader); // 建立一个对象，它把文件内容转成计算机能读懂的语言
    ...
    @ApiOperation(value = "删除地名文化内容", notes = "")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "地名文化id", required = true, dataType = "integer", paramType = "query"),
    ...
    private ResultVO del( HttpServletRequest request) throws IOException {
        ...
        placenacultuMapper.delPlacenacultu(id,currentUserId);
    ...
    private ResultVO update(@ApiParam("file") MultipartFile file, HttpServletRequest request) throws IOException {
        ...
        Placenacultu placenacultu=placenacultuMapper.getPlacenacultuById2(id,currentUserId);
        ...
        if(file!=null) {
            ...
            if(placenacultu!=null){

                if(placenacultu.getPictureurl()!=null&&!placenacultu.getPictureurl().equals("")) {
                    String[] strings = placenacultu.getPictureurl().split("\\/");

                    File file1 = new File(ConfigBean.sourceRealPath + "information" + File.separator + "Placenacultu" + File.separator + strings[5]);
            ...
            placenacultuMapper.updatePlacenacultu(name,imgVirtualPath,abbreviation,id,currentUserId);

            Placenacultu avillagerhyme2=placenacultuMapper.getPlacenacultuById2(id,currentUserId);
            ...
            placenacultuMapper.updatePlacenacultu(name,placenacultu.getPictureurl(),abbreviation,id,currentUserId);
